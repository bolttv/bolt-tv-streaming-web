...
export async function registerRoutes(
  ...
  // Auth0 resend verification email
  app.post("/api/auth/resend-verification", async (req, res) => {
    try {
      ...
      if (!tokenData.access_token) {
        console.error("Failed to get Auth0 Management API token:", tokenData);
      ...
      if (resendResponse.ok) {
        ...
        console.error("Failed to resend verification email:", errorData);
