...
export class MemStorage implements IStorage {
  ...
  private mediaCategoryMap: Map<string, string>;
  ...
  async updateWatchProgress(
    ...
    if (existing && existing.length > 0) {
      ...
      return this.mapWatchHistoryRow(updated);
    } else {
      ...
      if (error) throw error;
      return this.mapWatchHistoryRow(created);
    }
  }
  ...
  async getContinueWatching(sessionId: string, userId?: string): Promise<ContinueWatchingItem[]> {
    ...
    return filtered.map((item: any) => ({
      ...
      horizontalPosterLogo: getJWPlayerHorizontalPosterLogo(item.media_id),
      duration: item.duration,
  ...
  async getNextEpisodeToWatch(sessionId: string, seriesId: string, userId?: string): Promise<{ seasonNumber: number; episodeNumber: number; mediaId: string } | null> {
    ...
    if (supabaseAdmin && episodeMediaIds.length > 0) {
      let query = supabaseAdmin
        .from('watch_history')
        .select('*')
        .in('media_id', episodeMediaIds);
    ...
    for (const item of watchedItems) {
      watchProgressMap.set(item.media_id, item.progress);
    }
  ...
  async migrateSessionToUser(sessionId: string, userId: string): Promise<void> {
    ...
    for (const item of sessionItems) {
      ...
      if (existing && existing.length > 0) {
        if (new Date(item.last_watched_at) > new Date(existing[0].last_watched_at)) {
          await supabaseAdmin
            .from('watch_history')
            .update({
  ...
  private mapWatchHistoryRow(row: any): WatchHistory {
    return {
      id: row.id,
