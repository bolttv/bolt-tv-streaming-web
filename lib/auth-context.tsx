...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const [cleengCustomer, setCleengCustomer] = useState<CleengCustomer | null>(null);
  const [isLinking, setIsLinking] = useState(false);
  const [linkAttempted, setLinkAttempted] = useState(false);
  const migrationAttempted = useRef(false);

  useEffect(() => {
    const existing = getCleengCustomer();
    if (existing) setCleengCustomer(existing);
  }, []);
  ...
  const linkToCleeng = useCallback(async (currentUser: User) => {
    ...
    try {
      ...
      console.log("Cleeng SSO response received");
      ...
      if (response.jwt) {
        ...
        try {
          ...
          console.log("Cleeng subscriptions response received");
