...
export default function ContentDetails() {
  ...
  const { data: episodes = [] } = useSeriesEpisodes(isSeries ? id : undefined);

  // Fetch next episode to watch for series
  const { data: nextEpisode } = useNextEpisode(
    isSeries ? id : undefined,
    getSessionId(),
  );
  ...
  const isSingleContent =
    ...
    content?.contentType === "Documentary" ||
  ...
  const displayDescription =
    content.description ||
    "Experience the intensity and drama in this gripping series that takes you behind the scenes.";
        ...
        <div className="relative w-full h-[calc(75vh-60px)] sm:h-[calc(75vh-90px)] md:h-[calc(80vh-90px)] lg:h-[calc(90vh-90px)]">
          ...
          <div className="absolute top-0 bottom-0 left-4 right-4 sm:top-auto sm:right-auto sm:bottom-[60px] md:bottom-[80px] sm:left-4 md:left-12 max-w-xl z-20 flex flex-col justify-end sm:justify-end items-start text-left pb-[31px] sm:pb-0 space-y-2 sm:space-y-4 md:space-y-6">
            ...
            {(content.contentType === "Series" ||
              content.contentType === "Episode") && (
              <span className="text-blue-400 font-bold text-xs uppercase tracking-wider sm:hidden">
            ...
            <div className="flex items-center gap-2 sm:gap-3 text-[10px] sm:text-sm md:text-base font-medium text-gray-300 flex-wrap">
              <span className="bg-white/10 px-1 sm:px-1.5 py-0.5 rounded text-white border border-white/20">
                {content.rating}
              </span>
              {(content.contentType === "Series" ||
                content.contentType === "Episode") && <span>1 Season</span>}
              {(content.contentType === "Movie" ||
                content.contentType === "Documentary") && (
            ...
            <div className="w-[60%] sm:w-auto">
              <Link
                href={`/watch/${watchMediaId}${category ? `?category=${category}` : ""}`}
                className="w-full sm:w-auto"
              >
                <button
                  className="flex items-center justify-center gap-2 sm:gap-2 bg-white text-black hover:bg-white/90 transition-colors h-11 sm:h-10 md:h-12 w-full sm:w-auto px-4 sm:px-28 md:px-36 rounded font-semibold tracking-wide text-sm sm:text-sm md:text-base cursor-pointer"
            ...
            <div className="flex items-center gap-8 sm:gap-10 pt-2 sm:pt-3">
              ...
              {content.trailerId && (
                <Link
                  href={`/watch/${content.trailerId}${category ? `?category=${category}` : ""}`}
                >
                  <button className="flex flex-col items-center gap-1 text-gray-300 hover:text-white transition">
              ...
              <div className="px-4 md:px-12 pt-4 pb-8 space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
                  {episodes.map((ep, index) => (
                    <Link
                      key={ep.id}
                      href={`/watch/${ep.mediaId}${category ? `?category=${category}` : ""}`}
                    >
                      <div
                      ...
                      >
                        <div className="relative aspect-video bg-zinc-800 rounded-md overflow-hidden border-2 border-transparent transition-all duration-300 group-hover:border-white">
                          ...
                          <div className="absolute bottom-2 left-2 bg-black/60 px-1.5 py-0.5 rounded text-[10px] font-bold">
